...
module App = 
    ...
    let update (bluetooth: IBluetooth) msg model =
        match msg with
        | Increment ->
            bluetooth.CheckConnections()
    ...
    let program bluetooth =
        Program.mkProgram init (update bluetooth) view

type App (bluetooth: IBluetooth) as app = 
    inherit Application ()

    let runner = 
        App.program bluetooth
#if DEBUG
